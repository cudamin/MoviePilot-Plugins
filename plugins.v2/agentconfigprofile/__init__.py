...
class AgentConfigProfile(_PluginBase):
    ...
    将当前智能助手（Agent）的 LLM 供应商配置、鉴权信息与激活人格整体保存为命名模板，
    ...
    def init_plugin(self, config: dict = None):
        ...
        self._include_provider_auth = bool(config.get("include_provider_auth", True))
        self._include_persona = bool(config.get("include_persona", True))
    ...
    def _save_persistent_config(self, action_message: str = ""):
        ...
        self.update_config({
            ...
            "include_provider_auth": self._include_provider_auth,
            "include_persona": self._include_persona,
            "action": "none",
            "new_profile_name": "",
            "selected_profile": "",
    ...
    def _capture_snapshot(self) -> Dict[str, Any]:
        ...
        if self._include_provider_auth and self._include_credentials:
            agent_config = self._read_agent_config()
            provider_auth = agent_config.get("provider_auth")
            if isinstance(provider_auth, dict):
                snapshot["provider_auth"] = copy.deepcopy(provider_auth)

        if self._include_persona and agent_runtime_manager is not None:
            try:
                runtime_config = agent_runtime_manager.load_runtime_config()
                snapshot["active_persona"] = runtime_config.active_persona
            except Exception as err:
                logger.warning(f"智能助手配置模板：读取当前人格失败 - {err}")
    ...
    def _apply_snapshot(self, profile: Dict[str, Any]) -> str:
        ...
        provider_auth = snapshot.get("provider_auth")
        if isinstance(provider_auth, dict) and self._include_provider_auth and self._include_credentials:
            agent_config = self._read_agent_config()
            agent_config["provider_auth"] = copy.deepcopy(provider_auth)
            self._write_agent_config(agent_config)
            applied.append("鉴权信息")

        active_persona = snapshot.get("active_persona")
        if active_persona and self._include_persona and agent_runtime_manager is not None:
            try:
                agent_runtime_manager.set_active_persona(str(active_persona))
                applied.append(f"人格:{active_persona}")
            except Exception as err:
                logger.warning(f"智能助手配置模板：切换人格失败 - {err}")
    ...
    @staticmethod
    def _write_agent_config(value: Dict[str, Any]):
        """写回 AI 智能体配置（AIAgentConfig）。"""
        SystemConfigOper().set(SystemConfigKey.AIAgentConfig, copy.deepcopy(value) or None)
    ...
    def _current_summary(self) -> Dict[str, Any]:
        ...
        if agent_runtime_manager is not None:
            try:
                summary["active_persona"] = agent_runtime_manager.load_runtime_config().active_persona
            except Exception:
                pass
        return summary
    ...
    def _profile_is_active(self, profile: Dict[str, Any], current: Dict[str, Any]) -> bool:
        ...
        return (
            ...
            and snapshot.get("active_persona") == current.get("active_persona")
        )
        ...
        form = [
            {
                ...
                "props": {
                    ...
                    "text": "把当前智能助手的 LLM 配置与人格保存为模板，随时一键切换。"
        ...
        form.extend([
            ...
            },
            {
                ...
                "content": [
                    ...
                    {"component": "VCol", "props": {"cols": 12, "md": 3},
                     "content": [{"component": "VSwitch", "props": {"model": "include_provider_auth", "label": "含OAuth鉴权"}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 3},
                     "content": [{"component": "VSwitch", "props": {"model": "include_persona", "label": "含激活人格"}}]},
                ],
            },
    ...
    def get_page(self) -> List[dict]:
        ...
        header_chips = [
            ...
            {"component": "VChip", "props": {"color": "info", "variant": "flat", "class": "me-2"},
             "text": f"人格 {current.get('active_persona') or '-'}"},
            {"component": "VChip",
        ...
        for p in profiles:
            ...
            table_rows.append({
                ...
                "content": [
                    ...
                    {"component": "td", "text": snapshot.get("active_persona") or "-"},
                    {"component": "td", "text": "是" if llm.get("LLM_API_KEY") else "否"},
                    {"component": "td", "text": "是" if snapshot.get("provider_auth") else "否"},
            ...
            body = [{
                ...
                "content": [
                    {"component": "thead", "content": [{"component": "tr", "content": [
                        ...
                        {"component": "th", "text": "人格"},
                        {"component": "th", "text": "含密钥"},
                        {"component": "th", "text": "含鉴权"},

...
class SpaceCleaner(_PluginBase):
    ...
    _delete_seed_replicas = False  # 联动删除辅种（同 tmdbid 标签的所有种子）
    ...
    _rss_ntf = False
    _rss_th = 85
    _rss_dedup_earliest = False  # 同一集只下载最早发布的版本
    ...
    def init_plugin(self, config: dict = None) -> None:
        ...
        self._rss_th = 85
        self._rss_dedup_earliest = False
        ...
        self._rss_dedup_earliest = bool(config.get("rss_dedup_earliest"))
    ...
    def _update_config(self):
        self.update_config({
            ...
            "rss_th": self._rss_th, "rss_dedup_earliest": self._rss_dedup_earliest,
    ...
    def get_form(self) -> Tuple[Optional[List[dict]], Dict[str, Any]]:
        ...
        return [
                            ...
                            {"component": "VExpansionPanelText", "content": [
                                {
                                    ...
                                    "content": [
                                        ...
                                        {"component": "VRow", "content": [
                                            ...
                                            {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "delete_seed_replicas", "label": "联动删除辅种（同 tmdbid 标签）"}}]},
                                            {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "delete_same_name_torrents", "label": "删种同名种子", "hint": "触发删种时检索下载器，删除所有与被删种子同名的种子", "persistent-hint": True}}]},
                                        ]},
                                        {"component": "VRow", "content": [
                            ...
                            {"component": "VExpansionPanelText", "content": [
                                {
                                    ...
                                    "content": [
                                        {"component": "VRow", "content": [
                                            ...
                                            {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "rss_dedup_earliest", "label": "同一集只下载最早版本"}}]},
                                        ]},
                                        {"component": "VDivider", "props": {"class": "my-2"}},
                                        {"component": "VRow", "content": [
                                            {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VTextField", "props": {"model": "rss_th", "label": "跳过下载阈值(%)", "type": "number"}}]},
            ...
            "rss_once": False, "rss_ntf": True, "rss_th": 85, "rss_dedup_earliest": False,
        }
    ...
    def _rss_run(self):
        ...
        self._rss_busy = True
        try:
            if self._rss_dedup_earliest:
                # 开关开启：先收集所有 URL 的条目，统一去重后再下载
    ...
    def _rss_run_dedup(self, urls: List[str]):
        """收集所有 URL 的 RSS 条目，按 (tmdb_id, season, episode) 去重后统一下载。"""
            ...
            if cr["s"]:
                ...
                if self._rss_ntf:
                    self.post_message(title="SC-RSS跳过",
                                      text=f"{m.title} {se_fmt} {cr['r']}")
                continue
            # 同一集去重：开关开启时，同一 (tmdb_id, season, episode) 只保留 RSS feed 中最早出现的条目
            if self._rss_dedup_earliest and m.tmdb_id and meta.begin_episode:
                dedup_key = (m.tmdb_id, int(s_season), int(meta.begin_episode))

...
class SpaceCleaner(_PluginBase):
    plugin_name = "空间清理器"
    plugin_desc = "剩余空间不足时自动删除已观看资源（优先删除最早看完/标记的资源，合集需整季看完，含辅种及同集/同片的其他版本）；智能RSS下载自动跳过已看完剧集。"
        ...
        clean_form = {
            ...
            "content": [
                ...
                {"component": "VRow", "content": [
                    ...
                    {"component": "VCol", "props": {"cols": 6, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "delete_other_versions", "label": "删除其他版本", "hint": "删种时检索整理记录，删除同一集/同一部电影的其他版本（不同分辨率、字幕组等）", "persistent-hint": True}}]},
    ...
    def _delete_unit(self, unit, chain, space_info, all_torrents=None):
        ...
        if self._dry_run:
            extra = f"，另含其他版本 {len(other_versions)} 条" if other_versions else ""
        ...
        try:
            ...
            if other_versions:
                logger.info(f"已删除其他版本 {len(other_versions)} 条: {display_name}")
            extra = f"，含其他版本 {len(other_versions)} 条" if other_versions else ""
            self._add_delete_history(display_name, f"已删除（{len(records)} 条记录{extra}）")
            if self._notify:
                ver_line = f"\n其他版本: {len(other_versions)} 条" if other_versions else ""
    ...
    def _delete_downloader_torrents(self, chain, download_hash, display_name, all_torrents=None):
        ...
        否则残留的辅种会重新占用/锁定文件。此处扫描整个下载器，
        包含非 MP 管理的种子，因此非 MP 管理的辅种也会被删除。
        """
        if not download_hash:
            logger.warning(f"无 download_hash，跳过删种: {display_name}")
            return
        # 先定位主种子，取得其体积与名称作为辅种匹配依据
        ...
        # 删除辅种：体积一致且名称一致、hash 不同的种子（tracker 不同）
        ...
        for t in (torrents or []):
            ...
            if self._torrent_name(t) != main_name:
                continue
            t_disp = self._torrent_name(t) or h
            try:
                chain.remove_torrents(hashs=h, delete_file=True)
                logger.info(f"已删除辅种: {t_disp} ({h})")
            except Exception as e:
                logger.error(f"删除辅种失败 {t_disp}: {str(e)}")

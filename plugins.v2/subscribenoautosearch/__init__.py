...
class AgentConfigProfile(_PluginBase):
    ...
    将当前智能助手（Agent）的 LLM 供应商配置整体保存为命名模板，
    ...
    def init_plugin(self, config: dict = None):
        ...
        # 是否在快照中保存 LLM API Key（默认开启）
        # 兼容旧配置：老版本用 include_credentials 控制密钥，若新键缺失则回退到旧键
        self._save_api_key = bool(config.get("save_api_key", config.get("include_credentials", True)))
    ...
    def _capture_snapshot(self) -> Dict[str, Any]:
        ...
        for key in self._LLM_SETTING_KEYS:
            if key in self._SENSITIVE_SETTING_KEYS and not self._save_api_key:
                continue
        ...
        form.extend([
            {
                ...
                "content": [
                    ...
                    {"component": "VCol", "props": {"cols": 12, "md": 6},
                     "content": [{"component": "VSwitch", "props": {"model": "save_api_key", "label": "保存 API Key"}}]},
                ],
            },
            ...
            },
            {
                ...
                "props": {"type": "warning", "variant": "tonal", "class": "mt-2", "density": "compact",
                          "text": "开启“保存 API Key”后，模板会以明文保存密钥，请谨慎处理；"
                                  "关闭则模板不含密钥，应用时保留当前已有的 API Key。切换后新会话立即生效。"},
            },
        ])
